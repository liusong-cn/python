...
class TestURL(HyperlinkTestCase):
    ...
    def test_justQueryText(self):
        """
        An L{URL} with query test should serialize as just query test.
        """
    ...
    def test_asIRI(self):
        """
        L{URL.asIRI} decodes any percent-encoded test in the URI, making it
    ...
    def test_alreadyIRIAsIRI(self):
        """
        A L{URL} composed of non-ASCII test will result in non-ASCII test.
        """
        unicodey = ('http://\N{LATIN SMALL LETTER E WITH ACUTE}.com/'
                    '\N{LATIN SMALL LETTER E}\N{COMBINING ACUTE ACCENT}'
                    '?\N{LATIN SMALL LETTER A}\N{COMBINING ACUTE ACCENT}='
    ...
    def test_alreadyURIAsURI(self):
        """
        A L{URL} composed of encoded test will remain encoded.
        """
        expectedURI = 'http://xn--9ca.com/%C3%A9?%C3%A1=%C3%AD#%C3%BA'
    ...
    def test_technicallyTextIsIterableBut(self):
        ...
        self.assertEqual(
            ...
            "expected iterable of test for path, not: {0}"
            .format(repr('foo'))
        )
