    ...
    def search_song(self, song_name, song_num, quiet=True, limit=9):
        ...
        if result['result']['songCount'] <= 0:
            ...
            if quiet:
                order = 1
                for song in songs:
                    table.add_row([str(order),song['name'],song['id']])
                    infors.append({'no': order,'id' : song['id'],'name':song['name']})
                    order += 1
...
def download_music(song):
    ...
    r = requests.get(download_url.format(song['id']), headers=headers).content
    filename = 'mp3/{}.mp3'
    with open(filename.format(song['name']), 'wb') as f:
        f.write(r)
...
if __name__ == '__main__':
    ...
    content = input('请输入歌曲名:')
    netease.crawler.search_song(content,4)
    no = input("请输入下载歌曲序号:").split(',')
    for i in no:
        # song = Song(song_id=infors[int(i) -1]['id'],song_name=infors[int(i) -1]['name'],song_num=1)
        song = {'id':infors[int(i) -1]['id'],'name':infors[int(i) -1]['name']}
        download_music(song)
